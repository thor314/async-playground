...
impl TimerFuture {
  pub fn new(dur: Duration, s: String) -> Self {
    ...
    thread::spawn(move || {
      // do the sleeping. The executor is going to poll right about now, but may not poll until
      // after after this println. Note that Polling updates the waker field from None to
      // Some.
      println!(
    ...
    TimerFuture { shared_state: shared, name: s }
  }
}

// Okay, but we don't actually have a way to consume these futures yet. So write an executor.
// A what?
// An executor. it takes a Future, and runs it to completion. Futures, like teenagers are lazy.
// Executors make em move, like high school gym teachers. We're going to write a gym teacher. To do
// that, we need a utility from the futures crate.
pub mod executor {
  use {
  ...
  pub struct Task {
    // The future ain't here yet? It might be soon. We don't actually need this Mutex, since we'll
    // only have one thread executing tasks at a time. But compiler friend doesn't know that, so
    // the Mutex proves thread safety. In production, an `UnsafeCell` would be used to eliminate
    // that overhead.
    future:      Mutex<Option<BoxFuture<'static, ()>>>,
    task_sender: SyncSender<Arc<Task>>, /* woah dude, recurse. Handle to place the task back
                                         * onto the task queue. */
  }
  ...
  #[derive(Clone)]
  pub struct Spawner {
    // a SyncSender is the sending half of a channel. Someone's gonna receive it, and that
    // someone's a gym teacher.
  ...
  pub fn new_executor_and_spawner() -> (Executor, Spawner) {
    ...
    // create a new sync bounded channel. I think this is equivalent to mpsc::channel, with a
  ...
  impl Spawner {
    pub fn spawn(&self, future: impl Future<Output = ()> + 'static + Send) {
      ...
      // spawn the task, creating an atomic reference to pass it around. The task itself has to be
      // pinned to memory.
  ...
  // We want to be able to turn our tasks into Wakers. Wakers either bottom, or schedule a task to
  // be polled again.
  impl ArcWake for Task {
    fn wake_by_ref(arc_self: &Arc<Self>) {
      println!(
        "*5-ish* arcwake called from task. This gets called after the shared_state waker gets \
         taken."
      );
  ...
  // finally, our Executor will poll tasks by picking up the waker-ified tasks sent over the
  // channel.
  impl Executor {
    pub fn run(&self) {
      let mut count = 0;
...
fn main() {
  ...
  // if this line is not included, the last println never runs, and the executor will block
  // indefinitely, after the tasks are managed.
  drop(spawner);
